...
server.get(["/", "/team/:teamId"], async (req, res) => {
    const teamId = req.params.teamId;

    const { initialMarkup, initialData } = await serverRender(teamId);
...
server.get("*", async (req, res) => {
    const { initialMarkup, initialData } = await serverRender(req.params.teamId || null);
